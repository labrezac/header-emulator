...
from header_emulator.session import HeaderSession
...
def test_session_applies_rotated_headers():
...
def test_session_retries_on_retryable_status():
    calls: list[str] = []
...
def test_session_middleware_invocation():
    ...
    try:
        session.request("GET", "https://example.com/data")
    finally:
        session.close()
